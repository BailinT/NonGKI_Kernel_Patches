 ...
 * Return: 0 on success, an error code otherwise.
 */
extern bool odin_thermal_drop(int kind, unsigned long state);

static int cpufreq_set_cur_state(struct thermal_cooling_device *cdev,
				 unsigned long state)
{
	/* [odin-custom] perf mode ignores cpufreq cooling caps; an ignored
	 * cap request is translated into a release (also on repeated writes
	 * of the same state) so stale caps never linger. */
	if (odin_thermal_drop(0, state)) {
		if (state)
