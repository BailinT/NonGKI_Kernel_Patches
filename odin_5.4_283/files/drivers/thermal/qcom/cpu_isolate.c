...
static int cpu_isolate_set_cur_state(struct thermal_cooling_device *cdev,
				 unsigned long state)
{
	/* [odin-custom] perf mode ignores cpu isolation; an ignored request
	 * is translated into a release so stale isolation never lingers. */
	if (odin_thermal_drop(2, state)) {
		if (state)
			cpu_isolate_set_cur_state(cdev, 0);
		return 0;
	}
