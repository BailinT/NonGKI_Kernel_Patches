...
/* [odin-custom] exported clamp hook for odin_mode (module control) */
extern struct kgsl_device *kgsl_get_device(int dev_idx);

int kgsl_odin_clamp_max(unsigned int freq_hz)
{
	struct kgsl_device *device = kgsl_get_device(0);
	int level;
	...
	if (!device)
		return -ENODEV;
	level = _get_nearest_pwrlevel(&device->pwrctrl, freq_hz);
	if (level < 0)
		return level;
	kgsl_pwrctrl_set_thermal_pwrlevel(device, level);
	return 0;
}
EXPORT_SYMBOL(kgsl_odin_clamp_max);
