...
/* [odin-custom] GPU OC (900MHz) switch + insertion flag */
static bool odin_gpu_oc_enable;
static bool odin_gpu_oc_inserted;
module_param(odin_gpu_oc_enable, bool, 0644);
MODULE_PARM_DESC(odin_gpu_oc_enable,
	"ODIN: insert official 900MHz GPU level at top of pwrlevel table");

/* [odin-custom] perf-image option: drop the N lowest GPU pwrlevels
 * (315/379/443...) so DCVS never lingers at idle-ish frequencies. */
static int odin_gpu_low_drop;
module_param(odin_gpu_low_drop, int, 0644);
MODULE_PARM_DESC(odin_gpu_low_drop,
	"ODIN: number of lowest GPU pwrlevels to drop (perf image = 3)");
static bool odin_gpu_low_dropped;
		...
		struct device_node *node)
{
	...
	/* [odin-custom] perf-mode GPU OC: when the top DTS level is 840MHz,
	 * insert the official 900MHz level (voltage TURBO_L2=432, bus/acd as
	 * top level) at index 0 so perf mode can clock past 840. The 315MHz
	 * lowest level is preserved (table grows to 11 entries). */
	if (pwr->num_pwrlevels > 0 &&
	    pwr->num_pwrlevels < KGSL_MAX_PWRLEVELS &&
	    pwr->pwrlevels[0].gpu_freq == 840000000 && odin_gpu_oc_enable) {
		int oi;

		for (oi = pwr->num_pwrlevels; oi > 0; oi--)
			pwr->pwrlevels[oi] = pwr->pwrlevels[oi - 1];
		pwr->pwrlevels[0].gpu_freq = 900000000;
		pwr->pwrlevels[0].bus_freq = 11;
		pwr->pwrlevels[0].bus_min = 11;
		pwr->pwrlevels[0].bus_max = 11;
		pwr->pwrlevels[0].voltage_level = 9;
		pwr->num_pwrlevels++;
		pwr->active_pwrlevel++;
		pwr->default_pwrlevel++;
		pwr->min_pwrlevel++;
		odin_gpu_oc_inserted = true;
	}

	/* [odin-custom] drop the N lowest pwrlevels (perf image) */
	if (odin_gpu_low_drop > 0 && pwr->num_pwrlevels > odin_gpu_low_drop) {
		pwr->num_pwrlevels -= odin_gpu_low_drop;
		odin_gpu_low_dropped = true;
	}
